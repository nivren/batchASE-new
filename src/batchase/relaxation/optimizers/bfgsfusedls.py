...
class BFGSFusedLS:
    ...
    def step(self):
        ...
        for i in range(optimizable.batch_size):
            if not self.ls_completed[i]:
                continue
            mask = (i == optimizable.batch_indices)
    ...
    def update(self, r, g, r0, g0, p0_list):
        ...
        dr = r - r0
        dg = g - g0
        idx_t = self.optimizable.batch_indices.repeat_interleave(3)

        # ── Phase 1: collect indices that need Hessian update ──
        update_idx = []
        for i in range(self.optimizable.batch_size):
            if self.Hs[i] is None:
                self.Hs[i] = torch.eye(3 * all_sizes[i], device=self.device, dtype=self.dtype)
                continue
            if not self.ls_completed[i]:
                continue
        ...
        # ── Phase 2: group by matrix dimension, apply bmm or serial ──
        dim_groups = {}  # dim -> [indices]
        for i in update_idx:
            d = int(3 * all_sizes[i])
            dim_groups.setdefault(d, []).append(i)

        BMM_DIM_THRESHOLD = 600  # benchmark: bmm收益趋零拐点
        for dim, indices in dim_groups.items():
            if len(indices) > 1 and dim < BMM_DIM_THRESHOLD:
                self._update_bmm(indices, dr, dg, idx_t, dim)
            else:
                self._update_serial(indices, dr, dg, idx_t)

    def _update_bmm(self, update_idx, dr, dg, idx_t, dim):
        """Batched BFGS Hessian update via bmm for same-dimension structures."""
        M = len(update_idx)

        # Gather per-structure slices
        batch_H  = torch.stack([self.Hs[i] for i in update_idx])                   # (M, dim, dim)
        batch_dr = torch.stack([dr[i == idx_t].unsqueeze(-1) for i in update_idx])  # (M, dim, 1)
        batch_dg = torch.stack([dg[i == idx_t].unsqueeze(-1) for i in update_idx])  # (M, dim, 1)

        # rhok = 1 / (dg^T dr), keep (M,1,1) shape for direct broadcast
        dg_dr = torch.bmm(batch_dg.transpose(1, 2), batch_dr)   # (M, 1, 1)
        rhok = 1.0 / dg_dr
        rhok = torch.where(torch.isinf(rhok), torch.full_like(rhok, 1000.0), rhok)

        I = torch.eye(dim, device=self.device, dtype=self.dtype).unsqueeze(0)  # (1, dim, dim)

        # BFGS update: H = A1 @ H @ A2 + rhok * dr @ dr^T
        A1 = I - rhok * torch.bmm(batch_dr, batch_dg.transpose(1, 2))   # (M, dim, dim)
        A2 = I - rhok * torch.bmm(batch_dg, batch_dr.transpose(1, 2))   # (M, dim, dim)
        H_new = torch.bmm(A1, torch.bmm(batch_H, A2)) \
              + rhok * torch.bmm(batch_dr, batch_dr.transpose(1, 2))

        for j, i in enumerate(update_idx):
            self.Hs[i] = H_new[j]

    def _update_serial(self, update_idx, dr, dg, idx_t):
        """Serial BFGS Hessian update (fallback for single structure or large dim)."""
        for i in update_idx:
            mask = (i == idx_t)
            cur_dg = dg[mask]
            cur_dr = dr[mask]
            ...
            if torch.isinf(rhok):
                rhok = torch.tensor(1000.0, device=self.device, dtype=self.dtype)

            I = torch.eye(self.Hs[i].shape[0], device=self.device, dtype=self.dtype)
